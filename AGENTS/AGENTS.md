...
TLS integration should replace the legacy GUI automation path with a device wrapper
around the `tls_c1` SDK while preserving the same architecture rule:
...
- expose TLS status, errors, and movement state through `control/events.py` and `control/state.py`;
- keep future hardware smoke tests opt-in via environment variables.
- treat `Mr-enthalpy/tls_c1` as an optional external dependency rather than a vendored subtree.
...
- document installation as an external dependency from `https://github.com/Mr-enthalpy/tls_c1`;
- keep vendor DLL handling outside this repository and use `TLS_C1_SDK_DIR` on the local machine when needed.
...
- base `requirements.txt` may stay independent from `tls_c1` when the project must remain importable without the SDK;
- README should explain both `pip install git+https://github.com/Mr-enthalpy/tls_c1.git` and local editable install from a checkout;
- README should state that `TLS_C1_SDK_DIR` is local-machine configuration, not a committed asset path.
