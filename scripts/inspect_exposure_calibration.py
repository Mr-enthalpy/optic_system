#!/usr/bin/env python3
"""
Inspect Phase 3.0.5 exposure sweep HDF5.
...
def main() -> None:
    ...
    parser.add_argument(
        "h5_path", nargs="?", default="data/raw/bishe_exposure_sweep.h5",
        help="Path to exposure sweep HDF5",
    )
    parser.add_argument(
        "--json", default="outputs/exposure_calibration/camera_params.json",
    ...
    with h5py.File(h5_path, "r") as f:
        ...
        if not args.short:
            print(f"{'WL(nm)':>7s}  {'Exp(us)':>10s}  {'Gain(dB)':>8s}  {'Max':>7s}  "
                  f"{'P99.9':>7s}  {'SatFrac':>8s}  {'pSig':>7s}  {'Safe':>5s}  {'LowSig':>7s}")
            print("-" * 85)
            ...
            safe_arr = f["sweep/safe"][:]
            psig_arr = f["sweep/p_signal"][:]
            low_arr = f["sweep/low_signal"][:]

            for i in range(n):
                print(f"{wl_arr[i]:7.1f}  {exp_arr[i]:10.1f}  {gain_arr[i]:8.1f}  "
                      f"{max_arr[i]:7.1f}  {p99_arr[i]:7.1f}  {sf_arr[i]:8.4f}  "
                      f"{psig_arr[i]:7.1f}  {str(safe_arr[i]):>5s}  {str(low_arr[i]):>7s}")
    ...
    if json_path.exists():
        ...
        print(f"\n(camera_params.json not found at {json_path})")
