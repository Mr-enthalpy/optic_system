#!/usr/bin/env python3
"""Phase 3.2b PSF repeatability and mask-induced diversity raw capture."""
...
def run_capture_psf_repeatability(
    plan: dict[str, Any],
    *,
    dry_run: bool,
    ...
    validate_phase32_plan(plan, task="repeatability", hardware=not dry_run)
    ...
    if not dry_run:
        print("Phase 3.2b repeatability capture requires exclusive camera/LCD access.")
        lock.acquire()
    try:
        if dry_run:
        ...
        masks = representative_pupil_masks(
            physical_shape,
            pupil_window,
            subpixel_axis=subpixel_axis,
            include=list(plan["masks"]["include"]),
        ...
        run_status.update(
            ...
            phase="3.2b",
            task="psf_repeatability_capture",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Phase 3.2b PSF repeatability capture")
    parser.add_argument("--plan", default="plans/bishe_psf_repeatability.yaml")
    ...
    validate_phase32_plan(plan, task="repeatability", hardware=True)
