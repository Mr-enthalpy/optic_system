#!/usr/bin/env python3
"""Phase 3.2a camera-frame PSF ROI raw capture."""
...
def run_capture_psf_roi(
    ...
    *,
    ...
    validate_phase32_plan(plan, task="roi", hardware=not dry_run)
    ...
    writer: Phase32RawWriter | None = None

    if not dry_run:
        print("Phase 3.2a PSF ROI capture requires exclusive camera/LCD access.")
        lock.acquire()
    ...
    try:
        ...
        writer = Phase32RawWriter(output_raw, plan_id=plan["plan_id"], phase="3.2a").open()
        ...
        run_status.update(
            ...
            phase="3.2a",
            task="camera_frame_psf_roi_capture",
            current_stage="starting",
            current_mask_id=mask_meta["mask_id"],
            n_repeats=repeats,
            repeat_index=0,
        ...
        for repeat_index in range(repeats):
            run_status.update(current_stage="capture", repeat_index=repeat_index)
            if dry_run:
                frame = _synthetic_psf_frame(mask_id=mask_meta["mask_id"], repeat_index=repeat_index)
            else:
                lcd_service.show_mono_mask(mask, mask_id=mask_meta["mask_id"], mode="phase3_2_psf_roi")
                time.sleep(settle_s)
                capture = capture_adapter.acquire_burst(frames_per_capture)
                frame = np.asarray(capture.frames_avg, dtype=np.float64)
            writer.append_capture(frame_avg=frame, mask_id=mask_meta["mask_id"], repeat_index=repeat_index, mask_metadata=mask_meta)
            if repeat_index % max(1, int(status_preview_every)) == 0:
        ...
        run_status.update(current_stage="completed", completed=True)
        return output_raw
...
def _apply_pupil_shape_to_dry_lcd_meta(lcd_meta: dict[str, Any], pupil_window: dict[str, Any]) -> None:
    h, w = [int(v) for v in pupil_window["physical_shape"]]
    subpixel_axis = int(lcd_meta.get("subpixel_axis", 1))
    logical_shape = [h, w // 3] if subpixel_axis == 1 else [h // 3, w]
    lcd_meta["physical_shape"] = [h, w]
    lcd_meta["logical_shape"] = logical_shape
    lcd_meta["reported_shape"] = [logical_shape[0], logical_shape[1], 3]


def _synthetic_psf_frame(*, mask_id: str, repeat_index: int) -> np.ndarray:
    h, w = 256, 320
    yy, xx = np.mgrid[:h, :w]
    rng = np.random.default_rng(1000 + int(repeat_index))
    cx = 163.4 + 0.08 * repeat_index
    cy = 127.2 - 0.05 * repeat_index
    blob = 190.0 * np.exp(-(((xx - cx) / 13.0) ** 2 + ((yy - cy) / 10.0) ** 2))
    halo = 35.0 * np.exp(-(((xx - cx) / 34.0) ** 2 + ((yy - cy) / 28.0) ** 2))
    base = 12.0 + 0.03 * xx + 0.02 * yy
    noise = rng.normal(0.0, 0.8, size=(h, w))
    return (base + blob + halo + noise).astype(np.float64)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Phase 3.2a PSF ROI capture")
    parser.add_argument("--plan", default="plans/bishe_psf_roi.yaml")
    ...
    validate_phase32_plan(plan, task="roi", hardware=True)
    ...
    if tls_service is None and not args.allow_wavelength_labels_without_tls:
        print("ERROR: TLS service is required for hardware Phase 3.2 capture.", file=sys.stderr)
        sys.exit(1)
    try:
...
def _connect_tls(serial: str | None) -> Any:
    tls_serial = serial or os.environ.get("TLS_C1_SERIAL")
    if not tls_serial:
        return None
...
def _close_hardware(camera_service: Any, lcd_service: Any, tls_service: Any) -> None:
    for op in (camera_service.stop_stream, camera_service.close_camera, camera_service.close):
        try:
            op()
        except Exception:
            pass
