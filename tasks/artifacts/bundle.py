...
ARTIFACT_BUNDLE_SCHEMA_VERSION = CURRENT_BUNDLE_SCHEMA_VERSION
...
class ArtifactLocation:
    ...
    def from_dict(cls, data: Mapping[str, Any]) -> "ArtifactLocation":
        ...
        _reject_unknown_fields(data, {"storage_root", "rel_path"}, "artifact location")
        return cls(
...
class ArtifactPayload:
    ...
    def from_dict(cls, data: Mapping[str, Any]) -> "ArtifactPayload":
        ...
        _reject_unknown_fields(
            data,
            {"rel_path", "media_type", "size_bytes", "sha256"},
            "payload record",
        )
        return cls(
            rel_path=_required_string(data, "rel_path"),
...
class ArtifactBundleManifest:
    ...
    def validate(self) -> None:
        ...
        has_manifest_contract = self.artifact_type in CURRENT_MANIFEST_SCHEMA_VERSIONS
        has_payload_contract = self.artifact_type in CURRENT_PAYLOAD_SCHEMA_VERSIONS
        if not has_manifest_contract and not has_payload_contract:
            raise ArtifactBundleError("artifact_type is not supported")
        if has_manifest_contract:
            if isinstance(self.manifest_schema_version, bool) or not isinstance(
                self.manifest_schema_version,
                int,
            ):
                raise ArtifactBundleError(
                    "manifest_schema_version must be an integer for this artifact_type"
                )
            try:
                read_schema_version(
                ...
                raise ArtifactBundleError(
                    "artifact_type or manifest_schema_version is not supported"
                ) from exc
        elif self.manifest_schema_version is not None:
            raise ArtifactBundleError(
                "manifest_schema_version must be null when no manifest contract exists"
            )
        if has_payload_contract:
            try:
                read_payload_schema_version(
                ...
                raise ArtifactBundleError(
                    "artifact_type or payload_schema_version is not supported"
                ) from exc
        elif self.payload_schema_version is not None:
            raise ArtifactBundleError(
                "payload_schema_version must be null when no payload contract exists"
            )
    ...
    def from_dict(cls, data: Mapping[str, Any]) -> "ArtifactBundleManifest":
        ...
        return cls(
            ...
            manifest_schema_version=_required_optional_int(
                data,
                "manifest_schema_version",
            ),
            payload_schema_version=_required_optional_int(
                data,
                "payload_schema_version",
            ),
            payloads=payloads,
...
def validate_bundle(
    ...
    if not payload_result.ok:
        return _with_bundle_context(payload_result, bundle)
    if payload_result.manifest_schema_version != bundle.manifest_schema_version:
        return _bundle_result(
            artifact_type,
            ValidityOutcome.INVALID,
            bundle,
            "manifest_schema_version_mismatch",
            "primary payload manifest version does not match the bundle manifest",
        )
    if payload_result.payload_schema_version != bundle.payload_schema_version:
        return _bundle_result(
            artifact_type,
            ValidityOutcome.INVALID,
            bundle,
            "payload_schema_version_mismatch",
            "primary payload container version does not match the bundle manifest",
        )
...
def _validate_manifest_sidecar_consistency(
    ...
    *,
    ...
    if (
        primary_schema_version != bundle.manifest_schema_version
        or sidecar_schema_version != bundle.manifest_schema_version
...
def _load_bundle_for_validation(
    ...
    if isinstance(raw, Mapping) and isinstance(artifact_type, str):
        manifest_version = raw.get("manifest_schema_version")
        if (
            artifact_type in CURRENT_MANIFEST_SCHEMA_VERSIONS
            ...
            try:
                ...
                return _bundle_result(
                    ...
                    "bundle manifest contract requires a newer reader",
                    manifest_schema_version=manifest_version,
                    bundle_schema_version=bundle_schema_version,
                )
            except SchemaCompatibilityError:
                pass
        payload_version = raw.get("payload_schema_version")
        if (
            artifact_type in CURRENT_PAYLOAD_SCHEMA_VERSIONS
            and isinstance(payload_version, int)
            and not isinstance(payload_version, bool)
        ):
            try:
                read_payload_schema_version(payload_version, artifact_type)
            except NewerSchemaVersionError:
                return _bundle_result(
                    artifact_type,
                    ValidityOutcome.UNSUPPORTED,
                    ...
                    "bundle payload contract requires a newer reader",
                    manifest_schema_version=(
                        manifest_version
    ...
    try:
        ...
        manifest_version = (
            raw.get("manifest_schema_version") if isinstance(raw, Mapping) else None
        )
        payload_version = (
            raw.get("payload_schema_version") if isinstance(raw, Mapping) else None
        )
        bundle_version = (
            raw.get("bundle_schema_version") if isinstance(raw, Mapping) else None
        )
        return _bundle_result(
            artifact_type if isinstance(artifact_type, str) else "artifact_bundle",
            ValidityOutcome.INVALID,
            ...
            bundle_schema_version=(
                bundle_version
                if isinstance(bundle_version, int)
                and not isinstance(bundle_version, bool)
                else None
            ),
        )
...
def _bundle_result(
    ...
    bundle: ArtifactBundleManifest | None,
    reason_code: str,
    error: str,
    *,
...
def _with_bundle_context(
    result: ValidityResult,
    bundle: ArtifactBundleManifest,
) -> ValidityResult:
    """Preserve payload diagnostics while attaching the enclosing bundle identity."""
    return ValidityResult(
        artifact_type=result.artifact_type,
        outcome=result.outcome,
        representation=ArtifactRepresentation.BUNDLE,
        schema_version=bundle.manifest_schema_version,
        manifest_schema_version=bundle.manifest_schema_version,
        payload_schema_version=bundle.payload_schema_version,
        bundle_schema_version=bundle.bundle_schema_version,
...
def _required_optional_int(
    ...
    if field_name not in data:
        raise ArtifactBundleError(f"{field_name} is required")
    value = data[field_name]
    if value is None:
        return None
    if isinstance(value, bool) or not isinstance(value, int):
        raise ArtifactBundleError(f"{field_name} must be an integer or null")
    return value
...
def _reject_unknown_fields(
    ...
    if any(not isinstance(key, str) for key in data):
        raise ArtifactBundleError(f"{label} field names must be strings")
