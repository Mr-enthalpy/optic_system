...
def test_preconfig_gui_is_deprecated_json_reply() -> None:
    reply = impl.deprecated_preconfig_gui_reply()
...
def test_property_snapshot_conversion_is_json_safe() -> None:
    payload = impl.property_snapshot_to_dict(_fake_property_snapshot())
...
def test_frame_metadata_builder_raw8_raw16_and_rgb() -> None:
    raw8 = impl.frame_layout_from_array(np.zeros((3, 4), dtype=np.uint8), pixel_format="RAW8")
    raw8_meta = impl.build_frame_metadata(raw8, index=0, seq=1, ts_ns=10)
    ...
    raw16 = impl.frame_layout_from_array(np.zeros((3, 4), dtype=np.uint16), pixel_format="RAW16")
    raw16_meta = impl.build_frame_metadata(raw16, index=1, seq=2, ts_ns=20)
    ...
    rgb = impl.frame_layout_from_array(np.zeros((2, 3, 3), dtype=np.uint8), pixel_format="RGB8")
    rgb_meta = impl.build_frame_metadata(rgb, index=2, seq=3, ts_ns=30)
    ...
    inferred_rgb = impl.frame_layout_from_array(np.zeros((2, 3, 3), dtype=np.uint8))
...
def test_start_stop_stream_are_idempotent() -> None:
    ...
    state.shm = object()
    state.layout = impl.frame_layout_from_array(np.zeros((2, 2), dtype=np.uint8), pixel_format="RAW8")
...
def test_import_failure_open_camera_error_is_readable(monkeypatch) -> None:
    monkeypatch.setattr(impl, "FlyCapture2Camera", None)
    monkeypatch.setattr(impl, "_FLYCAPTURE2_IMPORT_ERROR", ImportError("No module named flycapture2_c"))
...
def test_known_but_undecoded_pixel_format_is_rejected_before_configuration(monkeypatch) -> None:
    ...
    monkeypatch.setattr(impl, "_fc2_support_for_pixel_format", fake_support)

    state = impl.CameraServiceState()
    backend = SimpleNamespace(
        stop_capture=lambda: None,
        start_capture=lambda: None,
        apply_config=lambda **kwargs: (_ for _ in ()).throw(AssertionError("should not configure")),
        _read_frame=lambda: impl.CapturedFrame(
        ...
        _discover_setting_names=lambda: [],
        layout=impl.frame_layout_from_array(np.zeros((2, 2), dtype=np.uint8), pixel_format="RAW8"),
    )
    state.cam = backend
