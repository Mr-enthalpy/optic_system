...
from tasks.artifacts.validation import read_validated_manifest_mapping
from tasks.profiles import PupilProfile
from tasks.psf.build_full_frame_psf_survey import FullFramePSFSurveyManifest
from tests.artifact_validation_helpers import _write_raw_capture
...
def _bundle_for_profile(tmp_path: Path) -> tuple[ArtifactBundleManifest, Path, Path]:
    ...
    bundle = ArtifactBundleManifest(
        ...
        artifact_type="pupil_profile",
        manifest_schema_version=schema_compat("pupil_profile").current,
...
def _survey_manifest() -> FullFramePSFSurveyManifest:
    ...
    return FullFramePSFSurveyManifest(
        ...
        source_raw_capture_artifact_id="raw_capture_bundle_v1",
...
def _write_survey_h5(path: Path, manifest: FullFramePSFSurveyManifest) -> None:
    with h5py.File(path, "w") as h5:
        ...
        h5.attrs["payload_schema_version"] = payload_schema_version(
            "full_frame_psf_survey"
        )
...
def _bundle_for_survey(
    ...
    *,
    ...
    bundle = ArtifactBundleManifest(
        ...
        manifest_schema_version=schema_compat("full_frame_psf_survey").current,
        payload_schema_version=payload_schema_version("full_frame_psf_survey"),
        payloads={
...
def test_bundle_json_roundtrip_and_local_payload_validation(tmp_path: Path) -> None:
    ...
    assert result.manifest_schema_version == bundle.manifest_schema_version
    assert result.payload_schema_version == bundle.payload_schema_version
    assert result.bundle_schema_version == bundle.bundle_schema_version
...
def test_raw_capture_bundle_uses_only_payload_and_bundle_version_axes(
    tmp_path: Path,
) -> None:
    generation = tmp_path / "raw_generation"
    generation.mkdir()
    raw_path = generation / "raw_capture.h5"
    _write_raw_capture(raw_path)
    bundle = ArtifactBundleManifest(
        artifact_id="raw_capture_generation_v1",
        artifact_type="raw_capture",
        manifest_schema_version=None,
        payload_schema_version=payload_schema_version("raw_capture"),
        payloads={
            "data": inspect_payload(
                raw_path,
                "application/x-hdf5",
                rel_path="raw_capture.h5",
            )
        },
    )
    ...
    assert result.payload_schema_version == payload_schema_version("raw_capture")
    assert result.bundle_schema_version == ARTIFACT_BUNDLE_SCHEMA_VERSION
    assert "schema_version" not in bundle.to_dict()


def test_bundle_compares_manifest_and_payload_versions_independently(
    tmp_path: Path,
) -> None:
    survey_bundle, survey_generation, _ = _bundle_for_survey(tmp_path)
    old_manifest_bundle = ArtifactBundleManifest(
        artifact_id=survey_bundle.artifact_id,
        artifact_type=survey_bundle.artifact_type,
        manifest_schema_version=1,
        payload_schema_version=survey_bundle.payload_schema_version,
        payloads=survey_bundle.payloads,
    )

    manifest_result = validate_bundle(old_manifest_bundle, survey_generation)

    assert manifest_result.outcome is ValidityOutcome.INVALID
    assert manifest_result.reason_codes == ("manifest_schema_version_mismatch",)

    raw_generation = tmp_path / "raw_mismatch_generation"
    raw_generation.mkdir()
    raw_path = raw_generation / "raw_capture.h5"
    _write_raw_capture(raw_path)
    historical_payload_bundle = ArtifactBundleManifest(
        artifact_id="raw_capture_generation_v2",
        artifact_type="raw_capture",
        manifest_schema_version=None,
    ...
    payload_result = validate_bundle(historical_payload_bundle, raw_generation)

    assert payload_result.outcome is ValidityOutcome.INVALID
    assert payload_result.reason_codes == ("payload_schema_version_mismatch",)


def test_manifest_reader_enforces_require_json_for_hdf5(tmp_path: Path) -> None:
    bundle, generation, _ = _bundle_for_survey(tmp_path)

    result = read_validated_manifest_mapping(
        bundle.artifact_type,
        generation / bundle.payloads["data"].rel_path,
        require_json=True,
    )

    assert not isinstance(result, tuple)
    assert result.outcome is ValidityOutcome.UNSUPPORTED
    assert result.reason_codes == ("representation.json_required",)
...
def test_bundle_with_newer_artifact_schema_is_unsupported(tmp_path: Path) -> None:
        ...
        json.dumps(
            {
                ...
                "artifact_type": "pupil_profile",
                "manifest_schema_version": schema_compat("pupil_profile").current + 1,
                "payload_schema_version": None,
                "payloads": {
                    "data": {
                        "rel_path": "profile.json",
    ...
    assert result.outcome is ValidityOutcome.UNSUPPORTED
    assert result.reason_codes == ("manifest_schema_newer_than_supported",)
